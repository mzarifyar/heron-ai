...
These are the tweaks required to run Cortex locally without access to AWS secret
...
- Copy `.env.example` to `.env` and populate the following keys:
  ...
  - `CORTEX_DISABLE_AI=1` – Skips GenAI agent calls when running offline.
...
- AI evaluation short-circuits when `CORTEX_DISABLE_AI` is set, returning a
